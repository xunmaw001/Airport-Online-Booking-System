<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="飞行方式" prop="feixingfangshi">
          <el-input v-model="ruleForm.feixingfangshi" 
              placeholder="飞行方式" clearable  :readonly="ro.feixingfangshi"></el-input>
        </el-form-item>
        <div v-else>
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="购票日期" prop="goupiaoriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.goupiaoriqi" 
                type="date"
                :readonly="ro.goupiaoriqi"
                placeholder="购票日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.goupiaoriqi" label="购票日期" prop="goupiaoriqi">
              <el-input v-model="ruleForm.goupiaoriqi" 
                placeholder="购票日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.goupiaoriqi = this.getCurDate()
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shenfenzhenghao = true;
          ...
          if(o=='goupiaoriqi'){
            this.ruleForm.goupiaoriqi = obj[o];
	    this.ro.goupiaoriqi = true;
            continue;
          }
        }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dingpiaoxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dingpiaoxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
